...
public class ValorBoard {
    ...
    private void initializeBoard() {
        ...
        for (int i = 0; i < obstacleCount; i++) {
            int row = random.nextInt(ValorBoardConstants.BOARD_SIZE - ValorBoardConstants.HEROES_NEXUS_ROW_OFFSET) + ValorBoardConstants.HEROES_NEXUS_ROW_OFFSET;
            int col;
            do {
                col = random.nextInt(ValorBoardConstants.BOARD_SIZE);
            } 
            while (ValorBoardUtilities.isWallColumn(col) || ValorBoardUtilities.isMonstersNexus(row) || ValorBoardUtilities.isHeroesNexus(row, ValorBoardConstants.BOARD_SIZE));
